...
class SuccessStoryCategoryWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_posts = SuccessStoryCategory::where(['status' => 'publish'])->orderBy('id', 'DESC')->take($post_items)->get();
        ...
        foreach ($blog_posts as $post) {
            $output .= '<li><a href="' . route('frontend.success.story.category', ['id' => $post->id,'any' => Str::slug($post->name)]) . '"><i class="fas fa-chevron-right"></i> ' . purify_html($post->name) . '</a></li>';
        }
        $output .= '</ul>';
    ...
    public function widget_title()
    {
        ...
        return __('Success Story Category');
    }
}