...
class RecentJobPostWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_posts = Jobs::where([ 'status' => 'publish'])->orderBy('id', 'DESC')->take($post_items)->get();
        ...
        foreach ($blog_posts as $post) {
            $output .= '<li class="single-recent-post-item">
                    <div class="content">
                        <h4 class="title"><a href="' . route('frontend.jobs.single', $post->slug) . '">' . purify_html($post->title ). '</a></h4>
